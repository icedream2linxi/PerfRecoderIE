// PerfRecorder.cpp : Implementation of CPerfRecorder
...
STDMETHODIMP CPerfRecorder::InterfaceSupportsErrorInfo(REFIID riid)
{
...
HRESULT CPerfRecorder::FinalConstruct()
{
	...
	m_thread = std::thread(&CPerfRecorder::run, this);
...
void CPerfRecorder::FinalRelease()
{
	m_stop = true;
	m_thread.join();
}

void CPerfRecorder::SetErrMsg(BSTR errMsg)
{
...
void CPerfRecorder::SetNvErr(NvAPI_Status status)
{
...
STDMETHODIMP CPerfRecorder::getPhysicalGPUIds(BSTR* ids)
{
	if (m_physicalGpuCount == 0)
	{
		NvAPI_Status res = NvAPI_EnumPhysicalGPUs(m_hPhysicalGpu, &m_physicalGpuCount);
...
STDMETHODIMP CPerfRecorder::getGPUFullName(LONG id, BSTR* name)
{
	NvAPI_ShortString szName;
...
STDMETHODIMP CPerfRecorder::getGPUUsage(LONG id, LONG* usages)
{
...
STDMETHODIMP CPerfRecorder::getGPUMemoryInfo(LONG id, BSTR* info)
{
	NvAPI_Status res = NVAPI_OK;
...
NvAPI_Status CPerfRecorder::getGPUMemoryInfo(NvPhysicalGpuHandle hPhysicalHandle, NV_DISPLAY_DRIVER_MEMORY_INFO &memoryInfo)
{
...
STDMETHODIMP CPerfRecorder::getAllProcessInfo(BSTR* info)
{
...
STDMETHODIMP CPerfRecorder::monitoringProcess(ULONG pid)
{
	m_processId = pid;
	m_sysNoIdleTime = 0;
	m_processNoIdleTime = 0;
...
STDMETHODIMP CPerfRecorder::getProcessCPUUsage(FLOAT* usage)
{
	*usage = m_cpuUsage;
	return S_OK;
}
...
float CPerfRecorder::getProcessCPUUsage()
{
...
STDMETHODIMP CPerfRecorder::getProcessMemoryInfo(BSTR* usage)
{
	MemoryInfo info = getProcessMemoryInfo();
...
CPerfRecorder::MemoryInfo CPerfRecorder::getProcessMemoryInfo()
{
...
STDMETHODIMP CPerfRecorder::getIfAdapters(BSTR* adapters)
{
	StringBuffer sb;
	Writer<StringBuffer> writer(sb);
...
STDMETHODIMP CPerfRecorder::selectIfAdapter(SHORT idx)
{
...
STDMETHODIMP CPerfRecorder::getNetTransSpeed(BSTR* netTransSpeed)
{
	StringBuffer sb;
	Writer<StringBuffer> writer(sb);
	writer.StartObject();
